...
def train(opt, logger):
    ...
    # Setup Metrics
    running_metrics_val_mr = runningScore(opt.n_class)
    running_metrics_val_ct = runningScore(opt.n_class)
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        for source_data in train_loader_src:
            ...
            if (i + 1) % opt.val_interval == 0:
                validation(model, logger, val_loader, device, running_metrics_val_mr, running_metrics_val_ct, iters = model.iter, opt=opt)
                torch.cuda.empty_cache()
                logger.info('Best iou until now is {}'.format(model.best_iou))

            model.scheduler_step()

def validation(model, logger, datasets, device, running_metrics_val_mr, running_metrics_val_ct, iters, opt=None):
    iters = iters
    _k = -1
    ...
    print('Source MR validation')
    with torch.no_grad():
        validate(datasets[0], device, model, running_metrics_val_mr)

    score, class_iou = running_metrics_val_mr.get_scores()
    ...
    score, class_iou = running_metrics_val_ct.get_scores()
    for k, v in score.items():
        print()
        print(k, v)
        logger.info('CT {}: {}'.format(k, v))

    for k, v in class_iou.items():
        logger.info('CT {}: {}'.format(k, v))

    running_metrics_val_mr.reset()
    running_metrics_val_ct.reset()
